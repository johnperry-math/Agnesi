...
/**
 * generates a path for an arrow, given two points (to indicate direction) and a length (to indicate size)
 * the arrow begins from (x0,y0), and [arrow_direction] computes the points
 */
...
abstract class Graph_Object(
    ...
    internal fun translate_point(
        ...
        x_scale: Double, y_scale: Double, x_translate: Double, y_translate: Double,
        height: Int
...
/**
 * contains the properties of a dynamic graph, which can include multiple plots;
 * the actual drawing is unloaded to [Plot_Points.draw_to]
 *
 * "Dynamic graph" means that one can drag or zoom on the graph and the plot(s) will react accordingly.
 *
 * @see Plot_Points
...
data class Graph_Properties(
    ...
    var x_min: Double = -6.0, var x_max: Double = 6.0,
    var y_min: Double = -4.0, var y_max: Double = 4.0,
    var arrow_size: Double = canvas.width / 30.0,
    val with_gridlines: Boolean = true, val grid_dx: Double = 1.0, val grid_dy: Double = 1.0,
    val grid_color: String = "#cccccc"
) {

    /**
     * the following properties convert from absolute x-, y-values to screen positions
     * and are used to help drag the graph
     */
    ...
    /**
     * determines [x_scale], [y_scale] for a zoom; also calls [translate] and sets [recently_modified]
     */
    fun rescale() {
     ...
     */
    @Suppress("unused")
    fun number_of_objects() = objects.size
    ...
    private fun draw_axes(context: CanvasRenderingContext2D, width: Double, height: Double) {
        ...
        // don't draw the axes if the graph has been translated to a point where they shouldn't be visible
...
/**
 * visual properties of a possibly dynamic plot in a graph; see [Function_Properties] for algebraic properties
 *
 * A plot is "dynamic" if it remakes its presentation whenever a graph's view changes by drag or zoom
 *
 ...
 * @property arrows whether to draw arrows on the left and the right: set corresponding component to true
 * @property connect_the_dots whether to connect the dots
 * @property holes known removable discontinuities (holes) in the plot; we draw holes and the curve skips over them
 * @property asymptotes known asymptotic discontinuities in the plot
 * @property jumps known jump discontinuities in the plot; corresponding [plugs] and [unplugs] would be useful
 * @property plugs dots to draw in the plot (think of plugging the holes of removable discontinuities, but this has other uses)
 * @property unplugs holes to draw in the plot; these are merely drawn and not considered when connecting dots (e.g., jump discontinuity)
 * @property change_with_view whether the plot is dynamic
 * TODO implement [connect_the_dots]
 *
 */
data class Plot_Points(
     ...
     */
    private var discontinuities = Array(holes.size + asymptotes.size + jumps.size) { Pair(disc_hole, 0.0) }
     ...
     *
     * Properties affected:
     * [last_translated_to], [plot_holes], [plot_plugs], [plot_unplugs], [plot_points], [discontinuities].
    ...
    private fun rescale_and_translate( properties: Graph_Properties) {
        ...
        points.mapTo(plot_points) { translate_point(it, x_scale, y_scale, x_translate, y_translate, height) }
        holes.mapTo(plot_holes) { translate_point(it, x_scale, y_scale, x_translate, y_translate, height) }
        plugs.mapTo(plot_plugs) { translate_point(it, x_scale, y_scale, x_translate, y_translate, height) }
        unplugs.mapTo(plot_unplugs) { translate_point(it, x_scale, y_scale, x_translate, y_translate, height) }
    ...
    override fun draw_to(properties: Graph_Properties) {
        ...
        while (i + 1 < plot_points.size) {
            ...
            if (j < discontinuities.size) {
                ...
                if (
                    discontinuities[j].first == disc_asymp &&
                    ...
                    dx /= scale
                    dy /= scale
                    var arrow_path = arrow_points(x, y, x + dx * 2.0, y + dy * 2.0, arrow_size, LEFT_HALF)
                    ...
                    // skip over points until we pass the asymptote and put some space between us and the asymptote

                    while (i + 1 < plot_points.size && plot_points[i].first - x_delta < discontinuities[j].second) i += 1
                    ...
                    if (i + 1 < plot_points.size) {
                        ...
                        dx /= scale
                        dy /= scale
                        arrow_path = arrow_points(x, y, x - dx * 2.0, y - dy * 2.0, arrow_size, RIGHT_HALF)
                ...
                else if (discontinuities[j].first == disc_hole && x_next + 2*x_delta > discontinuities[j].second) {
                ...
                else if (discontinuities[j].first == disc_jump && x_next + x_delta > discontinuities[j].second) {

                    context.lineTo(discontinuities[j].second - x_delta, y)
                    context.stroke()

                    context.beginPath()
                    while (i < plot_points.size && plot_points[i].first < discontinuities[j].second) i += 1
        ...
        if (arrows.first) {
            ...
            if ((!plot_plugs.any { abs(it.first - x0) < x_delta }) && (!plot_unplugs.any { abs(it.first - x0) < x_delta })) {
        ...
        if (arrows.second) {
            ...
            if ((!plot_plugs.any { abs(it.first - xn) < x_delta }) && (!plot_unplugs.any { abs(it.first - xn) < x_delta })) {
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (
            (color != other.color) || (thickness != other.thickness) || (dashed != other.dashed) ||
            (connect_the_dots != other.connect_the_dots) ||
            (disc_hole != other.disc_hole) || (disc_asymp != other.disc_asymp) ||
            (disc_jump != other.disc_jump) || (plot_points != other.plot_points) ||
            (last_translated_to != other.last_translated_to) || (plot_holes != other.plot_holes) ||
            (plot_plugs != other.plot_plugs) || (plot_unplugs != other.plot_unplugs) ||
            (!holes.contentEquals(other.holes)) || (!asymptotes.contentEquals(other.asymptotes)) ||
            (!jumps.contentEquals(other.jumps)) || (!plugs.contentEquals(other.plugs)) ||
            (!unplugs.contentEquals(other.unplugs)) || (!discontinuities.contentEquals(other.discontinuities))
        )
            return false
...
/**
 * computes the (absolute) points of a function described by [expr], starting at [a],
 * ending at [b], with [n] points
 */

fun plot(
    a: Double, b: Double, n: Int,
    expr: Evaluation_Tree_Node
...
/**
 * relates each canvas to a Graph_Properties
 * the [String] should be an id assigned to the canvas
 */
 ...
 */
fun find_events_gp(e: Event): Graph_Properties = my_properties[
        (e.target as HTMLCanvasElement).getAttribute("id")
]!!
 ...
 */
fun determine_Δx(x1: Double, y1: Double, x2: Double, y2: Double, gp: Graph_Properties): Pair<Double, Double> {
    var Δx = x2 - x1
    var Δy = y2 - y1
...
fun end_drag(e: Event) {
    ...
    val (Δx, Δy) =
        when (e) {
            is MouseEvent -> determine_Δx(e.x, props.start_drag.second, props.start_drag.first, e.y, props)
            is TouchEvent -> determine_Δx(
                e.changedTouches[0]!!.clientX.toDouble(), props.start_drag.second,
                props.start_drag.first, e.changedTouches[0]!!.clientY.toDouble(),
                props
            )
            else -> Pair(0.0, 0.0)
        }
    ...
    if (mouse_button_is_down) {
        ...
        if (e is MouseEvent) {
            ...
            if (e.changedTouches[0] != null) {
                val touch = e.changedTouches[0]!!
                find_events_gp(e).start_drag = Pair(touch.clientX.toDouble(), touch.clientY.toDouble())
...
/**
 * a [Graph_Object] that we use to label a function's graph;
 * this could be used for other things eventually
 ...
 * @param italics whether to italicize the text; the default is true because we ordinarily label with a letter
...
data class Text_Object(
    ...
    override fun draw_to(properties: Graph_Properties) {
        ...
        val y = if (point.second - ( metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent * 2 ) >= 0.0)
                    point.second - metrics.actualBoundingBoxDescent * 2
                 else
                     point.second + metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent
        /*context.fillRect(
 ...
 * @property change_with_view whether the plot should be dynamic; see [Plot_Points] for an explanation
 * @property num_points number of points to plot in the graph
 * @property holes list of x-, y-values of known removable discontinuities; see [Plot_Points] for more detail
 ...
 * @property unplugs holes to place on the curve for jump discontinuities; see [Plot_Points] for more detail
...
data class Function_Properties(
    ...
    var ind_var: String, var f: Evaluation_Tree_Node,

    var start: Double, var stop: Double,

    var color: String = "#0000ff", var thickness: Double = 1.0, var dashed: Boolean = false,
...
/**
 * client has redefined a function; identify and notify the function, then re-graph
 * the canvas and function must be made aware (via [which]'s data-input-box tag)
 * that the input box triggering [e] addresses them
 *
...
/**
 * client has redefined one or more (un-)plugs; identify and notify the function, then re-graph
 * the canvas and function must be made aware (via [which]'s data-input-box tag)
 * that the input box triggering [e] addresses them
 *
 * @param e event triggered by redefining the function in a text input
 ...
 */
fun apply(
    plot_canvas: Graph_Properties,
    fun_props: ArrayList<Function_Properties>
) {

    require(
        fun_props.all { ( it.start < it.stop ) and ( it.num_points > 2 ) }
    )
...
fun read_x_values(str: String): Array<Double> {
    ...
    return if (error) emptyArray()
       else Array(temp.size) { i -> temp[i] }
}

/**
 * read comma-delimited points from [str]; points are in the form (x,y),
 * where x and y satisfy format of [constant]
 */
...
fun new_graph(canvas_id: String) {
    ...
    for (i in 1.until(10)) {
        ...
        if (canvas.getAttribute("data-which-function$suffix") != null) {
            ...
            val f = if (input_box_name == "") parse(ind_var, which_function)
            else {
                val input_box = document.getElementById(input_box_name) as HTMLInputElement
                input_box.addEventListener("change", { e -> report_function_changed(e, canvas_id) }, false)
            ...
            val change_view = canvas.getAttribute("data-recompute-points$suffix")?.toBoolean() ?: false
            ...
            if (plug_box_name != "") {
                ...
                plug_box.addEventListener("change", { e -> report_plugs_changed(e, canvas_id) }, false)
            ...
            if (unplug_box_name != "") {
                ...
                unplug_box.addEventListener("change", { e -> report_plugs_changed(e, canvas_id, false) }, false)
            ...
            // put it together
            val f_props = Function_Properties(
                ind_var = ind_var, f = f,
            ...
            // add to records
            if (!fun_properties.containsKey(canvas_id))
                fun_properties[canvas_id] = arrayListOf()
